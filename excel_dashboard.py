...
st.set_page_config(page_title="Sales Dashbooard",
                   page_icon=":bar_chart:",
                   layout="wide")
...
customer_type = st.sidebar.multiselect(
    "Select the Customer_Type:",options=df["Customer_type"].unique(),default=df["Customer_type"].unique()
)
...
left_coulmn,middle_column,right_column=st.columns(3)
with left_coulmn:
    st.subheader("Total Sales:")
    st.subheader(f"US $ {total_sales:,}")
with middle_column:
    st.subheader("Average Ratiing:")
    st.subheader(f"US $ {average_rating} {star_rating}")
with right_column:
    st.subheader("average_sale_by_transaction:")
